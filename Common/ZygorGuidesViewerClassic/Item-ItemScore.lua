...
local function ItemScore_SetTooltipData(tooltip, tooltipobj)
	...
	if ZGV.db.profile.vendor_tooltip and not ItemScore.TooltipPatched then
		...
		if sellprice then
			tooltip:AddLine(" ")
			tooltip:AddLine("|cfffe6100Vendor price: "..ZGV.GetMoneyString(sellprice,nil,3).."|r")
		end
	end
...
function ItemScore:GetEquipmentSkills()
	...
	for i=1, GetNumSkillLines() do
		local skillName, header, _, skillRank, numTempPoints, skillModifier, skillMaxRank, isAbandonable, stepCost, rankCost, minLevel, skillCostType = GetSkillLineInfo(i);
