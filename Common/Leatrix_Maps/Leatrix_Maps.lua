	...
	----------------------------------------------------------------------
	-- 	Leatrix Maps 1.13.09 (10th August 2019, www.leatrix.com)
	----------------------------------------------------------------------
	...
	function LeaMapsLC:MainFunc()
		...
		----------------------------------------------------------------------
		-- Map position (not using this... yet)
		----------------------------------------------------------------------

		local function DoNotUncommentThisLine()

			-- Create container frame for WorldMapFrame
			local WorldMapClamp = CreateFrame("Frame", nil, UIParent)
			LeaMapsLC["WorldMapClamp"] = WorldMapClamp
			WorldMapClamp:SetSize(WorldMapFrame:GetSize())
			WorldMapClamp:SetMovable(true)
			WorldMapClamp:SetFrameStrata(WorldMapFrame:GetFrameStrata())

			-- Parent WorldMapFrame to container and allow movement
			WorldMapFrame:SetParent(WorldMapClamp)
			WorldMapFrame:ClearAllPoints()
			WorldMapFrame:SetPoint("CENTER", WorldMapClamp, "CENTER", 0, 0)
			WorldMapFrame:RegisterForDrag("LeftButton")

			-- Movement controls
			WorldMapFrame:SetScript("OnDragStart", function()
				if IsControlKeyDown() and IsShiftKeyDown() then
					WorldMapClamp:StartMoving()
				end
			end)

			WorldMapFrame:SetScript("OnDragStop", function()
				WorldMapClamp:StopMovingOrSizing()
				WorldMapClamp:SetUserPlaced(false)
				-- Save map frame position
				LeaMapsLC["MapPosA"], void, LeaMapsLC["MapPosR"], LeaMapsLC["MapPosX"], LeaMapsLC["MapPosY"] = WorldMapClamp:GetPoint()
			end)

			-- Save map position if map is closed
			...
			-- Position WorldMapFrame when shown
			hooksecurefunc(WorldMapFrame, "Show", function()
				WorldMapClamp:ClearAllPoints()
				WorldMapClamp:SetPoint(LeaMapsLC["MapPosA"], UIParent, LeaMapsLC["MapPosR"], LeaMapsLC["MapPosX"], LeaMapsLC["MapPosY"])
			end)

		end

		-- DoNotUncommentThisLine()
		...
		do
			...
			LeaMapsLC:MakeSL(scaleFrame, "MapScale", "Scale", "", 0.5, 1, 0.1, 36, -142, "%.1f")
		...
		do
			...
			LeaMapsLC:MakeSL(fadeFrame, "FadeLevel", "Opacity", "", 0, 1, 0.1, 36, -142, "%.1f")
	...
	function LeaMapsLC:SetDim()
		...
		if LeaMapsLC["FadeMap"] == "On" then
			LeaMapsCB["FadeMapBtn"]:Enable()
			LeaMapsCB["FadeMapBtn"]:SetAlpha(1.0)
		else
			LeaMapsCB["FadeMapBtn"]:Disable()
			LeaMapsCB["FadeMapBtn"]:SetAlpha(0.3)
		end
		if LeaMapsLC["RescaleMap"] == "On" then
			LeaMapsCB["RescaleMapBtn"]:Enable()
			LeaMapsCB["RescaleMapBtn"]:SetAlpha(1.0)
		else
			LeaMapsCB["RescaleMapBtn"]:Disable()
			LeaMapsCB["RescaleMapBtn"]:SetAlpha(0.3)
		end
	end
	...
	local function SlashFunc(str)
		...
		if str and str ~= "" then
			...
			elseif str == "wipe" then
				...
				ReloadUI()
			elseif str == "mapreset" then
				-- Reset map position
				-- LeaMapsLC["WorldMapClamp"]:ClearAllPoints()
				-- LeaMapsLC["WorldMapClamp"]:SetPoint("CENTER", UIParent, "CENTER", 0, 0)
	...
	eFrame:SetScript("OnEvent", function(self, event, arg1)
		...
		if event == "ADDON_LOADED" and arg1 == "Leatrix_Maps" then
			...
			LeaMapsLC:LoadVarNum("FadeLevel", 0.5, 0, 1)				-- Fade map level
			LeaMapsLC:LoadVarChk("RescaleMap", "Off")					-- Rescale map frame
			LeaMapsLC:LoadVarNum("MapScale", 1, 0.5, 1)					-- Map scale
	...
	LeaMapsLC:MakeCB(PageF, "FadeMap", "Fade map when moving", 16, -132, false)
	LeaMapsLC:MakeCB(PageF, "RescaleMap", "Rescale map frame", 16, -152, false)
